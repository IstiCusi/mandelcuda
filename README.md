...
with \( z_0 = 0 \) and \( c = x_0 + iy_0 \), where \( x_0 \) and \( y_0 \) are the real and imaginary parts of \( c \), 
calculated from pixel coordinates. During each iteration, we perform:
...
The process continues until either the magnitude of \( z_n \) squared (\( x^2 + y^2 \)) exceeds 4,
